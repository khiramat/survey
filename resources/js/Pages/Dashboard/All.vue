...
const selectedEventId = my_user_id;
...
function majority(id){
    ...
    Object.keys(props.events).forEach(function(key) {
        ...
        const total_point_ary = props.majorities.filter((u) => u.event_id === event.id);
        var last_obj = [];
...
function sum_total_point(id){
    ...
    Object.keys(props.events).forEach(function(key) {
        const event = this[key];
        const total_point_ary = props.sum_total_points.filter((u) => u.event_id === event.id);
        var last_obj = [];
        Object.keys(total_point_ary).forEach(function(key) {
            const val = this[key];
            last_obj.push(val.point);
...
function total_addition_f(id){
    ...
    Object.keys(props.events).forEach(function(key) {
        const event = this[key];
        const total_point_ary = props.total_addition.filter((u) => u.user_id === my_user_id && u.event_id === event.id);
        Object.keys(total_point_ary).forEach(function(key) {
            const val = this[key];
